module ActiveRecord
  ...
  module ConnectionAdapters
    class SeamlessDatabasePoolAdapter < AbstractAdapter
      ...
      # To be compatible with thinking sphinx, one must set the config to a working config. Since thinking 
      # sphinx only reads, we try to set the first read connection available, but if nu read pool is set, 
      # we return the master connection      
      ...
      # To be compatible with Thinking Sphinx; one must return the connection class name, since seamless_database_pool 
      # only supports one type of connection, it's safe to return the master connection
      def self.name
